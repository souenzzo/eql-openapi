     ...
     ::pco/resolve (fn [env input]
                     (let [params (pco/params env)
                           ...
                           kw (keyword (string/join "."
                                ...
                                "-raw")]
                       {output-ident {kw (response->content
                                           (assoc opts ::content content)
                                           response)}}))}))

(defn schema->operations
  [{::keys [base-name openapi]}
   [schema-name schema-object-or-reference]]
...
(defn resolvers-for
  [{::keys [openapi]
    :as    opts}]
  (let [operations (eql-openapi/operations openapi)
        schemas (get-in openapi ["components" "schemas"])]
    (concat
      (into []
        (comp (filter (comp #{"get"} ::eql-openapi/method))
          (map (partial openapi->resolver opts))
          (map pco/resolver))
        operations)
      (into []
        (comp
          (map (partial schema->operations opts))
          cat
          (map pco/resolver))
        schemas))))
